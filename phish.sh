...
menu(){
	printf "\e[0;36m1. Track IP"
  printf "\e[0;36m\n2. Exit"
  printf " \n"
	read -p $'What you wanna do?' option

	if [[ $option  == 1 ]]; then
	startserver
	elif [[ $option == 2 ]]; then
		printf "\e[0;33mTataa bye bye\n"
	sleep 1
...
printf "\e[0;36m -------------------------------\n"
 ...
 found(){
	...
	sleep5
	if [[ -s ngrok.log && -s ip.txt  ]]; then printf "Files check => +ve\n";
	 trackip
	fi
	 sleep 1
